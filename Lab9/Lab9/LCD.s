...
wait1	LDR R1, =SSI0_SR_R
		LDR R2, [R1]
		AND R2, #0x10			;Read SSI0_SR_R and check bit 4
		CMP R2, #0
		BNE wait1				;If bit 4 is high, loop back to step 1 (i.e. wait for BUSY bit to be low)
		
		LDR R1, =DC
		LDR R2, [R1]
		BIC R2, #0x40
		STR R2, [R1]			;Clear D/C=PA6 to zero
		
		LDR R1, =SSI0_DR_R
		STR R0, [R1]			;Write the command(which is contained in R0) to SSI0_DR_R
		
wait2	LDR R1, =SSI0_SR_R
		LDR R2, [R1]
		AND R2, #0x10			;Read SSI0_SR_R and check bit 4
		CMP R2, #0
		BNE wait2				;If bit 4 is high, loop back to step 5 (i.e. wait for BUSY bit to be low)  
...
step1	LDR R1, =SSI0_SR_R
		LDR R2, [R1]
		AND R2, #0x02
		CMP R2, #0			;Read SSI0_SR_R and check bit 1
		BEQ step1				;If bit 1 is low loop back to step 1 (wait for TNF bit to be high)

		LDR R1, =DC
		LDR R2, [R1]
		ORR R2, #0x40
		STR R2, [R1]			;Set PA6 to one
		
		LDR R1, =SSI0_DR_R
		STR R0, [R1]			;Write the 8-bit data(contained in R0) to SSI0_DR_R
